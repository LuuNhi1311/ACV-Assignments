...
def image_data_augmentation(mat, w, h, pleft, ptop, swidth, sheight, flip, dhue, dsat, dexp, gaussian_noise, blur,
    ...
    try:
        ...
        if (src_rect[0] == 0 and src_rect[1] == 0 and src_rect[2] == img.shape[0] and src_rect[3] == img.shape[1]):
            sized = cv2.resize(img, (w, h), interpolation=cv2.INTER_LINEAR)
        else:
            ...
            # resize
            sized = cv2.resize(cropped, (w, h), interpolation=cv2.INTER_LINEAR)
        ...
        sized = cv2.resize(mat, (w, h), interpolation=cv2.INTER_LINEAR)
...
class Yolo_dataset(Dataset):
    ...
    def _get_val_item(self, index):
        ...
        """
        img_path = self.imgs[index]
        bboxes_with_cls_id = np.array(self.truth.get(img_path), dtype=np.float64)
...
def get_image_id(filename:str) -> int:
    # PATCHED_IMAGE_ID: dataset filenames are hex hashes, not integers.
    # Return a stable integer derived from the basename so the COCO
    # evaluator has a one-to-one filename <-> image_id mapping.
    import hashlib, os
